...
st.markdown("Monitor latency benchmarks, context sizes, chunk similarity metrics, and compare local-first vs cloud AI modes.")
...
if not logs:
    ...
    fig_latency.add_trace(go.Scatter(
        x=df['timestamp'], y=df['retrieval_time'],
        mode='lines+markers', name='Retrieval Time',
        line=dict(color='#4b6b94', width=2),
        marker=dict(size=6)
    ))
    fig_latency.add_trace(go.Scatter(
        x=df['timestamp'], y=df['generation_time'],
        mode='lines+markers', name='Generation Time',
    ...
    with col_right:
        ...
        fig_scatter = px.scatter(
            df,
            x='context_length', y='avg_similarity',
            color='ai_mode',
            size='chunk_count',
    ...
    table_data = df[['timestamp', 'query', 'ai_mode', 'retrieval_time', 'generation_time', 'avg_similarity', 'context_length']].copy()
    ...
    table_data.columns = ['Timestamp', 'User Query', 'AI Mode', 'Retrieval Time', 'Gen Time', 'Avg Similarity', 'Context Length (chars)']
