...
with st.form("settings_form"):
    ...
    if ai_mode_selection == "Local Llama.cpp Mode" and not llama_ok:
        st.warning(
            "⚠️ Llama.cpp is not compiled or installed in this environment. The system will fall back to Gemini mode unless you resolve the dependencies."
        )
    ...
    selected_theme = st.selectbox("Application Custom Style:", options=theme_options, index=default_theme_idx)
...
st.write(
    "Clear local cache structures to free up storage or force the RAG engine to regenerate vector search responses."
)
