...
st.markdown("Select an indexed university PDF and generate a structured executive summary, including important policies, deadlines, and key highlights.")
...
if not docs:
    ...
    if st.session_state.active_summary_content and st.session_state.active_summary_name == selected_doc_name:
        ...
        with exp_col2:
            # Export PDF
            try:
                pdf_bytes = create_summary_pdf(st.session_state.active_summary_name, st.session_state.active_summary_content)
                st.download_button(
