...
@st.cache_resource
def get_cached_embedding_model(model_key: str):
...
class CachedLocalEmbeddings:
    """A wrapper for LangChain Embeddings that checks and writes to an SQLite cache."""
    
    def __init__(self, model_key="bge-small"):
        self.model_key = model_key
        self.model_name = SUPPORTED_MODELS.get(model_key, SUPPORTED_MODELS["bge-small"])
        self.underlying_embeddings = get_cached_embedding_model(model_key)
        
    def _get_hash(self, text: str) -> str:
        """Returns the MD5 hash of a text string."""
        return hashlib.md5(text.encode('utf-8')).hexdigest()
        
    def _lookup_cache(self, text_hash: str) -> list:
    ...
    def _write_cache(self, text_hash: str, vector: list):
    ...
    def embed_documents(self, texts: list) -> list:
        ...
        if uncached_texts:
            logger.info(f"Computing local embeddings for {len(uncached_texts)} uncached chunks using {self.model_key}...")
