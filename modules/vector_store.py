...
def get_index_dir() -> str:
    ...
    model_key = (
        st.session_state.get("embedding_model", "bge-small") if "embedding_model" in st.session_state else "bge-small"
    )
...
def get_active_embeddings() -> CachedLocalEmbeddings:
...
def load_vector_store() -> FAISS | None:
    ...
    Returns:
        Optional[FAISS]: Loaded FAISS vector store, or None if index doesn't exist.
    """
    path = get_index_path()
    ...
    try:
        ...
        vector_store = FAISS.load_local(
            folder_path=index_dir, embeddings=embeddings, index_name=INDEX_NAME, allow_dangerous_deserialization=True
        )
        val: FAISS = vector_store
        return val
